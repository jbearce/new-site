...
// disable oEmbed
function new_site_stop_loading_wp_embed() {
    if (!is_admin()) {
        wp_deregister_script("wp-embed");
    }
}
add_action("init", "new_site_stop_loading_wp_embed");
...
function new_site_make_scripts_async($tag, $handle, $src) {
    if (!is_admin()) {
...
add_filter("script_loader_tag", "new_site_make_scripts_async", 10, 3);
