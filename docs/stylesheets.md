...
If you're unfamiliar with the concept of `em` and `rem` units, the basic breakdown is that they are used to proportionality scale sizing based on the font size of the elements parent. As such, `1em` is equal to the `font-size` of the element it is applied to, or `16` if the element and none of its parents lack a set `font-size`. If this still doesn't make sense, check out [this article on SitePoint](https://www.sitepoint.com/understanding-and-using-rem-units-in-css/).
